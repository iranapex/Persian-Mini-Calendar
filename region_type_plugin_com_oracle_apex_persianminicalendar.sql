...
,p_default_workspace_id=>17581866622095633408
,p_default_application_id=>36979
,p_default_owner=>'RAYANICO'
);
end;
/
...
wwv_flow_api.create_plugin(
 p_id=>wwv_flow_api.id(49548643701290466930)
,p_plugin_type=>'REGION TYPE'
,p_name=>'COM.ORACLE.APEX.MINICALENDAR'
,p_display_name=>'Persian Mini Calendar'
,p_supported_ui_types=>'DESKTOP'
,p_plsql_code=>wwv_flow_utilities.join(wwv_flow_t_varchar2(
'--==============================================================================',
'-- Modified by :   Mahdi Ahmadi',
'-- Modified Date:  17 Bahman 1394',
'-- Website:        www.iranapex.ir',
'-- Email:          info@iranapex.ir',
'-- Github          https://github.com/iranapex/Persian-Mini-Calendar',
'--==============================================================================',
...
'    end if;',
'    ',
'    P_month := to_char(l_date , ''Month''); ',
'    ',
'    persian_month := CASE',
'                        WHEN P_month like ''%Farvardin%''   THEN ''فروردین''',
'                        WHEN P_month like ''%Ordibehesht%'' THEN ''اردیبهشت''',
'                        WHEN P_month like ''%Khordad%''     THEN ''خرداد''',
'                        WHEN P_month like ''%Tir%''         THEN ''تیر''',
'                        WHEN P_month like ''%Mordad%''      THEN ''مرداد''',
'                        WHEN P_month like ''%Shahrivar%''   THEN ''شهریور''',
'                        WHEN P_month like ''%Mehr%''        THEN ''مهر''',
'                        WHEN P_month like ''%Aban%''        THEN ''آبان''',
'                        WHEN P_month like ''%Azar%''        THEN ''آذر''',
'                        WHEN P_month like ''%Dey%''         THEN ''دی''',
'                        WHEN P_month like ''%Bahman%''      THEN ''بهمن''',
'                        WHEN P_month like ''%Esfand%''      THEN ''اسفند''',
'                        ELSE ''N/A''',
'                    END;',
'    ',
'    -- Print the calendar header.',
'    sys.htp.prn(''<div style="direction:rtl; font-family:Tahoma;" class="a-MiniCal">'');',
'    sys.htp.prn(''<h3 class="a-MiniCal-title">''||persian_month||'' ''||to_char(l_date,''YYYY'')||''</h3>'');',
'    sys.htp.prn(''<table class="a-MiniCal-month" summary="Calendar of ''||Persian_month||'' ''||to_char(l_date,''YYYY'')||''">'');',
'    sys.htp.prn(''<thead>'');',
'    sys.htp.prn(''<tr>'');',
'    ',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_SAT" title="شنبه">ش</th>'');',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_SUN" title="یکشنبه">ی</th>'');',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_MON" title="دوشنبه">د</th>'');',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_TUE" title="سه شنبه">س</th>'');',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_WED" title="چهارشنبه">چ</th>'');',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_THU" title="پنج شنبه">پ</th>'');',
'    sys.htp.prn(''<th class="a-MiniCal-dayOfWeek" scope="col" id="''||l_id||''_FRI" title="جمعه">ج</th>''); ',
'    ',
'    sys.htp.prn(''</tr>'');',
'    sys.htp.prn(''</thead>'');',
...
'    l_startdate := trunc(l_date,''MM'') - to_char(trunc(l_date,''MM''),''D'') ;',
'    l_enddate := last_day(l_date) + 7-to_char(last_day(l_date),''D'') - 1;',
...
'        else',
'            if to_char(l_startdate+l_daycount,''D'') = ''6'' then',
...
wwv_flow_api.create_plugin_attribute(
 p_id=>wwv_flow_api.id(51043540883284688992)
,p_plugin_id=>wwv_flow_api.id(49548643701290466930)
...
wwv_flow_api.create_plugin_attribute(
 p_id=>wwv_flow_api.id(51043541308578688993)
,p_plugin_id=>wwv_flow_api.id(49548643701290466930)
,p_attribute_scope=>'COMPONENT'
...
wwv_flow_api.create_plugin_file(
 p_id=>wwv_flow_api.id(51043544113448688996)
,p_plugin_id=>wwv_flow_api.id(49548643701290466930)
